    ...
    while True:
        ...
        if nivel == "A":
            if numeroDeDependentes == 0:
                salarioLiquido = salario - (salario * (3.0/100.0))
            else:
                salarioLiquido = salario - (salario * (8.0/100.0))
        elif nivel == "B":
            if numeroDeDependentes == 0:
                salarioLiquido = salario - (salario * (5.0/100.0))
            else:
                salarioLiquido = salario - (salario * (10.0/100.0))
        elif nivel == "C":
            if numeroDeDependentes == 0:
                salarioLiquido = salario - (salario * (8.0/100.0))
            else:  
                salarioLiquido = salario - (salario * (15.0/100.0))
        elif  nivel == "D": 
            if numeroDeDependentes == 0:
                salarioLiquido = salario - (salario * (10.0/100.0))
            else: 
