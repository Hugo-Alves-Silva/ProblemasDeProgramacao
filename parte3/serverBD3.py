...
pessoas = {
    'caio da silva': [7.5, 8.3, 9.0], 
    'mario da costa': [2.5, 7.0, 3.5],
    'pedro silva rodrigues': [0.0, 0.0, 0.0],
    'maria de souza': [8.6, 7.3, 9.9],
    'ana paula faria': [0.0, 0.0, 0.0]
}
...
def on_new_client(client, connection):
    ...
    dados = msg.decode()
    informacoes = pessoas.get(dados.lower()) 
    if(informacoes == None):
        reply = 'Pessoa não encontrada no banco de dados'
    else:
        reply = str(informacoes[0]) + "*" + str(informacoes[1])+ "*" + str(informacoes[2])
